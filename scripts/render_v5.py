...
(ROOT/'renders/v5').mkdir(parents=True,exist_ok=True)
(ROOT/'previews/v5').mkdir(parents=True,exist_ok=True)
...
if mode=='compat':
 ...
 prepare_frame(390);scene.render.filepath=str(ROOT/'previews/v5'/'compat45.png');t=time.time();bpy.ops.render.render(write_still=True);print('COMPAT45_DONE',round(time.time()-t,2),flush=True)
elif mode=='preview':
 ...
 for f in frames:
  prepare_frame(f);scene.render.filepath=str(ROOT/'previews/v5'/f'frame_{f:04d}.png');print('PREVIEW_FRAME',f,flush=True);t=time.time();bpy.ops.render.render(write_still=True);print('PREVIEW_DONE',f,round(time.time()-t,2),flush=True)
elif mode=='proof':
 ...
 frames=list(range(97,145))+list(range(205,253))+list(range(401,449))
 folder=ROOT/'previews/v5/acting-frames';folder.mkdir(parents=True,exist_ok=True)
 ...
 f=int(args[1]) if len(args)>1 else 420;prepare_frame(f);scene.render.filepath=str(ROOT/'renders/v5'/'hero-4k.png');bpy.ops.render.render(write_still=True)
 ...
 folder=ROOT/'renders/v5'/('frames' if mode=='final' else 'motion-test');folder.mkdir(parents=True,exist_ok=True)
 if mode=='final':
  stamp=ROOT/'renders/v5/frame-source.json'
  record={'scene':'observatory-v5.blend','scene_sha256':hashlib.sha256(Path(bpy.data.filepath).read_bytes()).hexdigest(),'samples':64,'resolution':[1920,1080],'fps':24,'adaptive_threshold':.025}
  if stamp.exists() and json.loads(stamp.read_text())!=record and any(folder.glob('[0-9]*.png')):raise RuntimeError('Scene or render settings changed: move previous V5 frames aside before resuming')
