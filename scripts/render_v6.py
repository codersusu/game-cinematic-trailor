...
(ROOT/'renders/v6').mkdir(parents=True,exist_ok=True)
(ROOT/'previews/v6').mkdir(parents=True,exist_ok=True)
...
if mode=='compat':
 ...
 prepare_frame(390);scene.render.filepath=str(ROOT/'previews/v6'/'compat45.png');t=time.time();bpy.ops.render.render(write_still=True);print('COMPAT45_DONE',round(time.time()-t,2),flush=True)
elif mode=='preview':
 ...
 for f in frames:
  prepare_frame(f);scene.render.filepath=str(ROOT/'previews/v6'/f'frame_{f:04d}.png');print('PREVIEW_FRAME',f,flush=True);t=time.time();bpy.ops.render.render(write_still=True);print('PREVIEW_DONE',f,round(time.time()-t,2),flush=True)
elif mode=='proof':
 ...
 frames=list(range(385,481))
 folder=ROOT/'previews/v6/acting-frames';folder.mkdir(parents=True,exist_ok=True)
 ...
 f=int(args[1]) if len(args)>1 else 420;prepare_frame(f);scene.render.filepath=str(ROOT/'renders/v6'/'hero-4k.png');bpy.ops.render.render(write_still=True)
 ...
 folder=ROOT/'renders/v6'/('frames' if mode=='final' else 'motion-test');folder.mkdir(parents=True,exist_ok=True)
 if mode=='final':
  stamp=ROOT/'renders/v6/frame-source.json'
  record={'scene':'observatory-v6.blend','scene_sha256':hashlib.sha256(Path(bpy.data.filepath).read_bytes()).hexdigest(),'samples':64,'resolution':[1920,1080],'fps':24,'adaptive_threshold':.025}
  if stamp.exists() and json.loads(stamp.read_text())!=record and any(folder.glob('[0-9]*.png')):raise RuntimeError('Scene or render settings changed: move previous V6 frames aside before resuming')
