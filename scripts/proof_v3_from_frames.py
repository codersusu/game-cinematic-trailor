...
from encode_v3 import CREDIT
folder=ROOT/'renders/v3/frames'
(ROOT/'previews/v3').mkdir(parents=True,exist_ok=True)
for f in range(49,97):
 with Image.open(folder/f'{f:04d}.png') as im:
  assert im.size==(1920,1080)
  im.verify()
out=ROOT/'previews/v3/Celestial-motion-1080p.mp4'
partial=out.with_name('.'+out.stem+'.partial.mp4')
bar=round((1080-1920/2.39)/2)
filters=f'drawbox=x=0:y=0:w=iw:h={bar}:color=black:t=fill,drawbox=x=0:y=ih-{bar}:w=iw:h={bar}:color=black:t=fill,scale=out_color_matrix=bt709:out_range=tv,format=yuv420p,setparams=range=limited:color_primaries=bt709:color_trc=bt709:colorspace=bt709'
...
(ROOT/'previews/v3/motion-qa.json').write_text(json.dumps({'source_frames':[49,96],'decoded_frames':count,'metadata':meta,'audio_start_seconds':2,'output':str(out.relative_to(ROOT))},indent=2))
print(out)
