"""An authored astronomical instrument for The Last Observatory.
...
def _animate(ob,start,end):
    for frame,rot in [(1,start),(288,start),(480,end),(576,end)]:
...
def build_armillary(center=(0,2,3.7),bronze_mat=None,dark_mat=None,accent_mat=None):
    ...
    restrained warm engravings. Nothing emits light. Rings settle by frame480.
    """
    ...
    pivots=[]
    settings=[(2.42,.185,.15,(pi/2,0,0),(pi/2,0,0)),
              (2.155,.145,.105,(pi/2,.32,-.37),(pi/2,.04,-.12)),
              (1.87,.135,.10,(pi/2,-.58,.55),(pi/2,-.28,.16)),
              (1.58,.105,.085,(.53,.13,-.14),(.80,.0,.0))]
    romans=['XII','I','II','III','IV','V','VI','VII','VIII','IX','X','XI']
    for n,(r,width,depth,start,end) in enumerate(settings):
        pivot=_empty('Ring %d | balanced pivot'%(n+1),root); pivots.append(pivot)
        ...
        for s in (-1,1):
            caps.cylinder((s*r,0,-depth*.85),(s*r,0,depth*.85),.079,24)
            caps.cylinder((s*r,0,depth*.85),(s*r,0,depth*1.1),.052,24)
            screws.box((s*r,0,depth*1.11),(.059,.009,.003))
        caps.finish('Ring %d | trunnion bearings'%(n+1),bronze,pivot,.004,True)
        screws.finish('Ring %d | screw slots'%(n+1),dark,pivot)

    # Paired gimballed spindle bridges connect each adjacent ring at the
    # intersection of their planes. Their motion is baked, so the saved scene
    # remains portable without Python handlers or runtime dependencies.
    for pair in range(3):
        ...
        rods=[]
        for sign in (-1,1):
            holder=_empty('Gimbal bridge %d %s | baked axis'%(pair+1,sign),root)
            ...
            rods.append((sign,holder))
        last=None
        for frame in sorted(set([1,288,480,576]+list(range(300,481,12)))):
            bpy.context.scene.frame_set(frame)
            n1=outer.rotation_euler.to_matrix()@Vector((0,0,1))
            n2=inner.rotation_euler.to_matrix()@Vector((0,0,1))
            axis=n1.cross(n2)
            if axis.length<.0001:axis=inner.rotation_euler.to_matrix()@Vector((1,0,0))
            axis.normalize()
            if last is not None and axis.dot(last)<0:axis=-axis
            last=axis.copy()
            for sign,holder in rods:
                d=axis*sign
    ...
    globe_root=_empty('Celestial globe | polar axis',root)
    _animate(globe_root,(0,-.20,-.10),(0,-.20,.10))
    ...
    globe.parent=globe_root; globe.location=(0,0,0); globe.data.materials.append(dark)
    ...
    lines.finish('Celestial globe | brass meridians',gold,globe_root,smooth=True)
    ...
    stars.finish('Celestial globe | constellation studs',gold,globe_root,smooth=True)
    links.finish('Celestial globe | constellation lines',gold,globe_root,smooth=True)
    spindle=Batch()
    ...
    base.finish('Armillary | cast pedestal and cradle',bronze,root,.012,True)
