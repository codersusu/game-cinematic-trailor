...
print(json.dumps({
    ...
    'reused_frames': 343 if (folder / 'reused-frames.json').exists() else 0,
    'new_frames_completed': len(done), 'latest_frame': latest,
    'shot': 'second approach' if latest and latest < 425 else 'hand reach' if latest and latest < 481 else 'closing' if latest else 'preparing',
    ...
    'movie_ready': (ROOT / 'The Last Observatory - Astra Approach.mp4').exists(),
