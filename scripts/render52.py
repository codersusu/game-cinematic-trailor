...
if mode=='preview':
 ...
 for f in frames:
  scene.frame_set(f);scene.render.filepath=str(ROOT/'previews'/f'frame_{f:04d}.png');print('PREVIEW_FRAME',f,flush=True);t=time.time();bpy.ops.render.render(write_still=True);print('PREVIEW_DONE',f,round(time.time()-t,2),flush=True)
elif mode=='hero':
 ...
 f=int(args[1]) if len(args)>1 else 420;scene.frame_set(f);scene.render.filepath=str(ROOT/'renders'/'hero-4k.png');bpy.ops.render.render(write_still=True)
