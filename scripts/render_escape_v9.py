...
sc.render.engine='BLENDER_EEVEE_NEXT';sc.eevee.taa_render_samples=8;sc.render.resolution_x=768;sc.render.resolution_y=432;sc.render.resolution_percentage=100;sc.render.image_settings.file_format='PNG';sc.render.use_motion_blur=False;sc.render.use_compositing=False;sc.render.use_sequencer=False;sc.render.use_simplify=True;sc.render.simplify_subdivision=1
folder=ROOT/'previews/v9'/view/('frames' if mode=='motion' else 'stills');folder.mkdir(parents=True,exist_ok=True)
identity={'scene':Path(bpy.data.filepath).name,'sha256':hashlib.sha256(Path(bpy.data.filepath).read_bytes()).hexdigest(),'view':view,'frames':144,'fps':12,'seconds':12,'resolution':[768,432],'engine':'EEVEE','samples':8,'heavy_render':False}
