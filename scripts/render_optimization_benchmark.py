...
try:
 prefs.compute_device_type='METAL';prefs.metalrt='OFF';prefs.kernel_optimization_level='FULL';prefs.get_devices()
...
def prepare_frame(frame):
 # The keeper lies well below the opening macro camera. Exclude his rig there
 # to avoid rebuilding its dense geometry; restore it for every later shot.
 def find_collection(layer):
  if layer.name=='CH-einar': return layer
  for child in layer.children:
   found=find_collection(child)
   if found is not None:return found
  return None
 keeper=find_collection(bpy.context.view_layer.layer_collection)
 if keeper is not None and keeper.exclude != (frame<=144):keeper.exclude=frame<=144
...
if mode=='compat':
 ...
 prepare_frame(390);scene.render.filepath=str(ROOT/'previews'/'compat45.png');t=time.time();bpy.ops.render.render(write_still=True);print('COMPAT45_DONE',round(time.time()-t,2),flush=True)
elif mode=='preview':
 ...
 for f in frames:
  prepare_frame(f);scene.render.filepath=str(ROOT/'previews'/f'frame_{f:04d}.png');print('PREVIEW_FRAME',f,flush=True);t=time.time();bpy.ops.render.render(write_still=True);print('PREVIEW_DONE',f,round(time.time()-t,2),flush=True)
elif mode=='hero':
 ...
 f=int(args[1]) if len(args)>1 else 420;prepare_frame(f);scene.render.filepath=str(ROOT/'renders'/'hero-4k.png');bpy.ops.render.render(write_still=True)
 ...
 folder=ROOT/'previews'/'optimization-benchmark';folder.mkdir(parents=True,exist_ok=True)
 for f in range(start,end+1):
  path=folder/f'{f:04d}.png'
  if path.exists():continue
  prepare_frame(f);scene.render.filepath=str(path);t=time.time();bpy.ops.render.render(write_still=True)
