...
movie=ROOT/'The Last Observatory.mp4'
gen=imageio_ffmpeg.read_frames(str(movie),pix_fmt='rgb24')
meta=next(gen);w,h=meta['size'];count=0;selected={48,120,180,240,336,420,492,540};thumbs=[]
for frame in gen:
 count+=1
...
sheet.save(ROOT/'previews/final-contact-sheet.jpg',quality=94)
...
(ROOT/'renders/movie-qa.json').write_text(json.dumps(report,indent=2))
