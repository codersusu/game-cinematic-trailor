...
if mode=='compat':
 ...
 scene.frame_set(390);scene.render.filepath=str(ROOT/'previews'/'compat45.png');t=time.time();bpy.ops.render.render(write_still=True);print('COMPAT45_DONE',round(time.time()-t,2),flush=True)
elif mode=='preview':
 ...
 for f in frames:
  scene.frame_set(f);scene.render.filepath=str(ROOT/'previews'/f'compat45_{f:04d}.png');print('PREVIEW_FRAME',f,flush=True);t=time.time();bpy.ops.render.render(write_still=True);print('PREVIEW_DONE',f,round(time.time()-t,2),flush=True)
elif mode=='hero':
 ...
 f=int(args[1]) if len(args)>1 else 420;scene.frame_set(f);scene.render.filepath=str(ROOT/'renders'/'hero45-4k.png');bpy.ops.render.render(write_still=True)
 ...
 folder=ROOT/'renders'/('frames45' if mode=='final' else 'motion-test45');folder.mkdir(parents=True,exist_ok=True)
 for f in range(start,end+1):
  path=folder/f'{f:04d}.png'
  if path.exists():continue
  scene.frame_set(f);scene.render.filepath=str(path);t=time.time();bpy.ops.render.render(write_still=True)
