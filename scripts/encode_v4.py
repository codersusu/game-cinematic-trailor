...
CREDIT = ('Character: ' + CHARACTER_CREDIT + '. '
          'Microsoft Rocketbox Female Adult 04 and native motion library, MIT; adapted Cycles materials and authored facial performance. Original procedural stellar galaxy. Environment scans: Poly Haven, CC0. '
...
def main():
    ...
    if args.prepare_overlays:
        make_overlays()
        print(ROOT/'renders/v4/title-card-preview.png')
        return
    final = args.mode == 'final'
    folder = ROOT/'renders/v4'/('frames' if final else 'motion-test')
    first, count, (w,h) = inspect_frames(folder, final)
    title_path, credits_path = make_overlays(w,h)
    out = ROOT/('The Last Observatory - Wonder.mp4' if final else 'previews/v4/motion-test.mp4')
    ...
    if final:
        soundtrack = ROOT/'audio/v4/last_observatory_v4_mix.wav'
