...
CREDIT = ('Character: ' + CHARACTER_CREDIT + '. '
          'Microsoft Rocketbox Female Adult 04 and native motion library, MIT; adapted Cycles materials and authored facial and V6 reaching performance. Original procedural stellar galaxy. Environment surfaces: ambientCG and Poly Haven, CC0. Reused sci-fi parts: Irondust and rubberduck/a52, CC0; custom orbital architecture and machine. '
...
def make_overlays(w=1920, h=1080):
    ...
    tracked(title, 'ASTRA REACH', h*.535, sans, w*.0105, w*.0027, (185,193,191,255))
...
def main():
    ...
    if args.prepare_overlays:
        make_overlays()
        print(ROOT/'renders/v6/title-card-preview.png')
        return
    final = args.mode == 'final'
    folder = ROOT/'renders/v6'/('frames' if final else 'motion-test')
    first, count, (w,h) = inspect_frames(folder, final)
    title_path, credits_path = make_overlays(w,h)
    out = ROOT/('The Last Observatory - Astra Reach.mp4' if final else 'previews/v6/motion-test.mp4')
