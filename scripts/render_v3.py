...
if mode=='compat':
 ...
 prepare_frame(390);scene.render.filepath=str(ROOT/'previews/v3'/'compat45.png');t=time.time();bpy.ops.render.render(write_still=True);print('COMPAT45_DONE',round(time.time()-t,2),flush=True)
elif mode=='preview':
 ...
 for f in frames:
  prepare_frame(f);scene.render.filepath=str(ROOT/'previews/v3'/f'frame_{f:04d}.png');print('PREVIEW_FRAME',f,flush=True);t=time.time();bpy.ops.render.render(write_still=True);print('PREVIEW_DONE',f,round(time.time()-t,2),flush=True)
elif mode=='hero':
 ...
 f=int(args[1]) if len(args)>1 else 420;prepare_frame(f);scene.render.filepath=str(ROOT/'renders/v3'/'hero-4k.png');bpy.ops.render.render(write_still=True)
 ...
 folder=ROOT/'renders/v3'/('frames' if mode=='final' else 'motion-test');folder.mkdir(parents=True,exist_ok=True)
