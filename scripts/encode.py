...
CREDIT = ('Character: Einar Rig (CC-BY 4.0), Blender Foundation, '
          'https://studio.blender.org/characters/einar/v1/ . '
          'License: https://creativecommons.org/licenses/by/4.0/ . '
          'Adapted materials, pose and animation. Environment scans and sky: Poly Haven, CC0. '
...
def make_overlays(w=1920, h=1080):
    ...
    tracked(credits, 'Einar Rig (CC-BY) Blender Foundation | studio.blender.org',
            h*.896, sans, w*.0115, w*.00012, (188,194,194,255))
    tracked(credits, 'Poly Haven scans: CC0   |   Sound effects: ElevenLabs',
            h*.928, sans, w*.0105, w*.00012, (160,169,170,255))
    folder = ROOT/'renders'
...
def main():
    ...
    if args.prepare_overlays:
        make_overlays()
        print(ROOT/'renders/title-card-preview.png')
        return
    final = args.mode == 'final'
    folder = ROOT/'renders'/('frames' if final else 'motion-test')
    first, count, (w,h) = inspect_frames(folder, final)
    title_path, credits_path = make_overlays(w,h)
    out = ROOT/('The Last Observatory.mp4' if final else 'previews/motion-test.mp4')
    ...
    if final:
        soundtrack = ROOT/'audio/last_observatory_mix.wav'
