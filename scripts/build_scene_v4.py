"""Reproducible cinematic environment. Blender --background --python scripts/build_scene_v4.py"""
...
for i in range(12):
 if i in [3,7,8,9,10]:continue
 # A broad north arch gives the stellar nucleus a clear night-sky backdrop.
 a1=TAU*i/12;a2=TAU*(i+(2 if i==2 else 1))/12
 ...
 ang=(a1+a2)/2
 wall=cube('Weathered parapet',(8.6*math.cos(ang),2+8.6*math.sin(ang),1.45),(8.1 if i==2 else 4.05,.7,2.5),stone,.055);wall.rotation_euler.z=ang+math.pi/2
...
for a in [.38,2.65]:
 ...
 basis=o.shape_key_add(name='Still folds')
 for phase in range(4):
  key=o.shape_key_add(name='Mountain breeze '+str(phase))
  for idx,vert in enumerate(key.data):
   free=(idx//9)/18
   vert.co.y+=.065*free*math.sin((idx%9)*.7+free*3+phase*math.pi/2)
  for frame in range(1,578,12):
   key.value=max(0,math.cos((frame-1)*TAU/168-phase*math.pi/2))*.65
...
# Keep a clean walking corridor; rain-polished paving continues beyond the chamber.
for ob in list(bpy.data.objects):
 if ob.name.startswith('Scanned floor gravel') and abs(ob.location.x+3.8)<1.0 and ob.location.y<0:
  bpy.data.objects.remove(ob,do_unlink=True)
for row in range(10):
 for col in range(4):
  cube('Entrance causeway stone',(-5.3+col*1.0,-9.35+row*.72,.035),(.985,.705,.21),floor,.015)
# Old south portal, standing within the broken colonnade. Clear span2.6m.
for side in [-1,1]:
 for row in range(3):
  for k in range(3):
   cube('Ruined entrance wing',(-3.8+side*(2.2+k*.78),-5.8,.35+row*.42),(.77,.70,.41),stone,.025)

for side in [-1,1]:
 x=-3.8+side*1.56
 cube('Portal foundation',(x,-5.8,.27),(.88,1.20,.28),stone,.05)
 for j in range(7):
  cube('Portal jamb ashlar',(x,-5.8,.62+j*.37),(.63,.82,.355),stone,.025)
 cube('Portal impost',(x,-5.8,3.12),(.84,1.02,.25),bronze,.025)
for j in range(19):
 aa=j*math.pi/19+.007;bb=(j+1)*math.pi/19-.007
 vs=[]
 for yy in [-6.23,-5.37]:
  for rr in [1.26,1.86]:
   for ang in [aa,bb]:vs.append((-3.8+rr*math.cos(ang),yy,3.03+rr*math.sin(ang)))
 bevel(mesh_obj('Portal crown voussoir',vs,[(0,1,3,2),(4,6,7,5),(0,4,5,1),(2,3,7,6),(0,2,6,4),(1,5,7,3)],stone),.022)
cube('Brass threshold inlay',(-3.8,-5.8,.151),(2.58,.14,.026),gold,.01)
# Both heavy leaves are already open, leaving the arrival uninterrupted.
for side in [-1,1]:
 hinge=bpy.data.objects.new('Open ancient door hinge',None);COL.objects.link(hinge)
 hinge.location=(-3.8+side*1.25,-5.86,.16);hinge.rotation_euler.z=-side*math.radians(76)
 for k in range(6):
  panel=cube('Bronze door plank',(-side*(k+.5)*.20,0,1.41),(.19,.13,2.82),dark,.022);panel.parent=hinge
 for z in [.28,1.18,2.52]:
  rail=cube('Door engraved binding',(-side*.60,-.083,z),(1.23,.045,.13),bronze,.018);rail.parent=hinge
  for x in [.15,.45,.75,1.05]:
   stud=cyl('Door hammered rivet',(-side*x,-.115,z),.035,.025,gold,16);stud.rotation_euler.x=math.pi/2;stud.parent=hinge
# Lamps provide a warm threshold against the cold stellar sky.
lantern_glow=material('Lantern amber flame',(1,.24,.04),0,.5)
lantern_glow.node_tree.nodes.get('Principled BSDF').inputs['Emission Color'].default_value=(1,.29,.055,1)
lantern_glow.node_tree.nodes.get('Principled BSDF').inputs['Emission Strength'].default_value=5
for side in [-1,1]:
 x=-3.8+side*1.64
 cyl('Portal lamp bracket',(x,-5.18,2.07),.22,.08,bronze,24)
 cyl('Portal lamp heart',(x,-5.18,2.34),.085,.4,lantern_glow,24)
 for k in range(4):
  aa=k*TAU/4
  line('Portal lantern cage',[(x+.16*math.cos(aa),-5.18+.16*math.sin(aa),zz) for zz in [2.10,2.62]],.018,bronze)
 cyl('Portal lantern crown',(x,-5.18,2.65),.22,.12,bronze,24)
...
mesh_obj('Airborne illuminated dust',verts,faces,gold).hide_render=True
...
try:
 from armillary_v2 import build_armillary
...
# Clear the mechanical center for a suspended spiral of individual stars.
# Outer two gimbals retain their original animation and engraved detail.
for obj in bpy.data.objects:
 lineage=[]; ancestor=obj
 while ancestor:
  lineage.append(ancestor.name); ancestor=ancestor.parent
 if any(name.startswith(('Ring 3 |','Ring 4 |','Celestial globe |','Polar spindle |',
                         'Gimbal bridge 2 ', 'Gimbal bridge 3 ')) for name in lineage):
  obj.hide_render=True;obj.hide_viewport=True
from galaxy_v3 import build_galaxy
galaxy=build_galaxy(center=(0,2,3.7))
keeper={}
if os.environ.get('OBS_SKIP_KEEPER')!='1':
 from heroine_motion_v4 import add_heroine_v4
 from heroine_look_v4 import apply_heroine_look, add_facial_performance, add_instrument_gaze, face_center
 keeper=add_heroine_v4(report_path=ROOT/'renders/v4/native-motion-report.json')
 apply_heroine_look(keeper['objects'])
 facial_report=add_facial_performance(keeper)
 add_instrument_gaze(keeper)
 (ROOT/'renders/v4/facial-performance.json').write_text(json.dumps(facial_report,indent=2))
print('KEEPER_INFO', {k:keeper[k] for k in ('face_target','foot_contacts') if k in keeper})
...
core=light('Awakening lens bounce','POINT',(0,.6,3.7),20,(.48,.72,1),.28)
for f,e in [(1,12),(380,18),(460,45),(480,90),(510,55),(576,45)]:core.data.energy=e;core.data.keyframe_insert('energy',frame=f)
light('Keeper eye catchlight','AREA',(-2.8,-2.1,2.1),15,(.8,.88,1),.6,target=(-2.8,-.3,1.8))
# Entry pools and a broad soft key keep her face and planted feet readable.
for side in [-1,1]:light('Threshold lantern light','POINT',(-3.8+side*1.64,-5.10,2.4),65,(1,.42,.13),.24)
light('Arrival soft moonlight','AREA',(-1.8,-4.0,5.7),500,(.65,.77,1),4,target=(-3.8,-5.8,1))
light('Keeper arrival fill','AREA',(0,-2.7,3.5),160,(1,.78,.55),3,target=(-3.8,-3.5,1.2))
...
def camera(name,start,end,target1,target2,lens,f1,f2,fstop=4):
 ...
 return o
camera('01 • Across the threshold',(-1.4,-1.2,2.55),(1.4,-.7,2.6),(-3.8,-6.7,1.15),(-3.8,-.9,1.15),30,37,192,7.1)
camera('02 • The turning heavens',(-3.8,-1.9,5.9),(-3.0,-1.5,5.6),(-.2,2,3.95),(.1,2,3.95),48,193,288,5.6)
wide=camera('03 • Under a billion stars',(7.6,-12,3.3),(5.7,-10.5,3.1),(0,2,4.0),(0,2,4.25),29,345,480,8)
camera('04 • The stars remember',(1.2,-12.4,3.9),(.6,-11.8,4.3),(0,2,4.45),(0,2,4.75),30,481,576,8)
if keeper:
 first_face=face_center(keeper,15)
 last_face=face_center(keeper,36)
 camera('00 • A breath at the threshold',first_face+Vector((.45,2.3,.09)),last_face+Vector((.33,2.13,.065)),first_face,last_face,72,1,36,5.6)
 wonder_start=face_center(keeper,289)
 wonder_end=face_center(keeper,344)
 camera('03A • Wonder',wonder_start+Vector((1.43,1.70,.085)),wonder_end+Vector((1.35,1.56,.08)),wonder_start,wonder_end,75,289,344,5.6)
 light('Threshold portrait lantern','AREA',(-4.9,-5.6,2.25),45,(1,.69,.38),1.1,target=tuple(first_face))
 light('Galaxy reaction key','AREA',(-2.5,1.3,2.4),25,(.58,.77,1),1.2,target=tuple(wonder_start))
 light('Reaction warm bounce','AREA',(-5.5,-1.5,2.6),30,(1,.70,.45),1.5,target=tuple(wonder_start))
scene.camera=wide
for f,exposure in [(1,-.55),(288,-.55),(289,-.85),(344,-.85),(345,-.55),(576,-.55)]:
 scene.view_settings.exposure=exposure;scene.view_settings.keyframe_insert('exposure',frame=f)
for curve in scene.animation_data.action.fcurves:
 if curve.data_path=='view_settings.exposure':
  for point in curve.keyframe_points:point.interpolation='CONSTANT'
# Selective stellar halo preserves the contrast of the surrounding masonry.
from compositor_v3 import configure_compositor
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/('observatory-v4-environment.blend' if os.environ.get('OBS_SKIP_KEEPER')=='1' else 'observatory-v4.blend')),compress=True)
