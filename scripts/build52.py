...
scene.camera=wide
# Subtle highlight bloom; Blender 5 compositor is a node-group datablock.
try:
 nt=bpy.data.node_groups.new('Observatory • optical finish','CompositorNodeTree')
 nt.interface.new_socket(name='Image',in_out='OUTPUT',socket_type='NodeSocketColor')
 scene.compositing_node_group=nt
 rl=nt.nodes.new('CompositorNodeRLayers');gl=nt.nodes.new('CompositorNodeGlare');gl.inputs['Type'].default_value='Fog Glow';gl.inputs['Quality'].default_value='High'
 if 'Threshold' in gl.inputs:gl.inputs['Threshold'].default_value=2.0
 if 'Strength' in gl.inputs:gl.inputs['Strength'].default_value=.13
...
except Exception as e:print('COMPOSITOR',e)
scene.frame_set(390)
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'observatory.blend'))
