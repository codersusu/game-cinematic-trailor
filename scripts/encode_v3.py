...
CHARACTER_CREDIT = os.environ.get('OBS_CHARACTER_CREDIT', 'Original heroine • Meshy 7 Ultra').strip() or 'Original heroine • Meshy 7 Ultra'
CREDIT = ('Character: ' + CHARACTER_CREDIT + '. '
          'Original imagegen concept; adapted materials, pose and animation. Original procedural stellar galaxy. Environment scans: Poly Haven, CC0. '
...
def make_overlays(w=1920, h=1080):
    ...
    # Two readable lines in the bottom matte. Final character credit is supplied at runtime.
...
def main():
    ...
    if args.prepare_overlays:
        make_overlays()
        print(ROOT/'renders/v3/title-card-preview.png')
        return
    final = args.mode == 'final'
    folder = ROOT/'renders/v3'/('frames' if final else 'motion-test')
    first, count, (w,h) = inspect_frames(folder, final)
    title_path, credits_path = make_overlays(w,h)
    out = ROOT/('The Last Observatory - Celestial.mp4' if final else 'previews/v3/motion-test.mp4')
