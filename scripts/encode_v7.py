...
CHARACTER_CREDIT = os.environ.get('OBS_CHARACTER_CREDIT', 'Character + native walk • Microsoft Rocketbox').strip() or 'Character + native walk • Microsoft Rocketbox'
CREDIT = ('Character: ' + CHARACTER_CREDIT + '. '
          'Microsoft Rocketbox Female Adult 04 and native motion library, MIT; adapted Cycles materials and authored facial and V7 approach and reaching performance. Original procedural stellar galaxy. Environment surfaces: ambientCG and Poly Haven, CC0. Reused sci-fi parts: Irondust and rubberduck/a52, CC0; custom orbital architecture and machine. '
...
def make_overlays(w=1920, h=1080):
    ...
    tracked(title, 'ASTRA APPROACH', h*.535, sans, w*.0105, w*.0027, (185,193,191,255))
...
def main():
    ...
    if args.prepare_overlays:
        make_overlays()
        print(ROOT/'renders/v7/title-card-preview.png')
        return
    final = args.mode == 'final'
    folder = ROOT/'renders/v7'/('frames' if final else 'motion-test')
    first, count, (w,h) = inspect_frames(folder, final)
    title_path, credits_path = make_overlays(w,h)
    out = ROOT/('The Last Observatory - Astra Approach.mp4' if final else 'previews/v7/motion-test.mp4')
    ...
    if final:
        soundtrack = ROOT/'audio/v7/last_observatory_v7_mix.wav'
