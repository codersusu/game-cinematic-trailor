...
for frame in [1,8,15,22,64,130,240,252]:
 ...
 if frame in [1,8,15,22]:
  scene.render.filepath=str(out/f'{frame:04d}.png');bpy.ops.render.render(write_still=True)
scene.frame_set(64);pos=result['root'].location.copy();cam.data.ortho_scale=.55;cam.location=pos+Vector((.3,3,1.57));cam.rotation_euler=(pos+Vector((0,0,1.5))-cam.location).to_track_quat('-Z','Y').to_euler();scene.render.filepath=str(out/'face.png');bpy.ops.render.render(write_still=True)
